...
shorten_method_01 = "shorten last S bits"
shorten_method_02 = "shorten last"
shorten_method_03 = "puncture first"
shorten_method_04 = "low-complexity short"
shorten_method_05 = "low-complexity puncture"
...
def conv1bit_getNextStates(in_bit, cur_state1, cur_state2, gen1, gen2, bit_flag):
    ...
    g_bit = in_bit       
    #gen1[0]+gen2[0] is avoided in both cases of bit_flag == 1 and bit_flag == 0
    #keeping the bits in either register, not in both
    if bit_flag == 1:
...
def conv_encodeR(in_code, gen, m):
    # function to find the convolutional code for given input code (input code must be padded with zeros)
    #cur_state = np.zeros(m, dtype=np.int)         # intial state is [0 0 0 ...]
    cur_state = [0 for i in range(m)]#np.zeros(m, dtype=int)
    len_in_code = len(in_code)           # length of input code padded with zeros
...
def generate_critical_set(frozen_bits:int):
    ...
    A = -1 * np.ones((n + 1, N), dtype=int)    #an extra row for frozen_bits
    for ii in range(N):
